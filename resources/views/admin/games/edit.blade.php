        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.games.update", [$game->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('team_one') ? 'is-invalid' : '' }}" name="team_one_id" id="team_one_id" required>
                        @foreach($team_ones as $id => $entry)
                            <option value="{{ $id }}" {{ (old('team_one_id') ? old('team_one_id') : $game->team_one->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('team_two') ? 'is-invalid' : '' }}" name="team_two_id" id="team_two_id" required>
                        @foreach($team_twos as $id => $entry)
                            <option value="{{ $id }}" {{ (old('team_two_id') ? old('team_two_id') : $game->team_two->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('avenue') ? 'is-invalid' : '' }}" type="text" name="avenue" id="avenue" value="{{ old('avenue', $game->avenue) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('game_date') ? 'is-invalid' : '' }}" type="text" name="game_date" id="game_date" value="{{ old('game_date', $game->game_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control timepicker {{ $errors->has('game_time') ? 'is-invalid' : '' }}" type="text" name="game_time" id="game_time" value="{{ old('game_time', $game->game_time) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('ticket_url') ? 'is-invalid' : '' }}" type="text" name="ticket_url" id="ticket_url" value="{{ old('ticket_url', $game->ticket_url) }}" required>
